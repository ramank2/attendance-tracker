...
function renderSummary(){
 ...
 const cols=[
  ...
  ["WL","Weekend"],["Present Eq.","Present Equivalent"],["Recorded","Recorded"]
 ];
 ...
 employees().filter(e=>`${e.id} ${e.name} ${e.designation}`.toLowerCase().includes(filter)).forEach(e=>{
   ...
   const workingDays=Math.max(0,hol.calendar-hol.weekend-hol.nh-hol.fl);
   const pct=workingDays?Math.min(100,presentEq(c)/workingDays*100):0;
   h+=`<tr><td><b>${e.id}</b></td><td style="text-align:left">${e.name||""}</td><td>${e.designation||""}</td>`+
      cols.map(col=>`<td>${col[0]==="Present Eq."?presentEq(c).toFixed(1):col[0]==="Recorded"?c.recorded:(c[col[0]]||0)}</td>`).join("")+
