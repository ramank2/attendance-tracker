...
- Employee Master is reconciled by Employee ID every time the app loads.
- Missing configured employees are restored automatically.
- Existing locally stored attendance remains preserved because attendance is keyed by Employee ID.
- Employees manually added in a browser are retained.


# Multi-device cloud sync (Supabase)

The tracker now supports central cloud storage through Supabase.
...
5. Commit `supabase-config.js` together with the other files and wait for GitHub Pages to deploy.
6. Open the tracker and click **Sign In** -> **Create Account**.
7. Use the same account on your laptop, phone and other devices.

## What is synchronized
- Employee Master
- Daily attendance
- National/Festival Holiday Calendar
- Custom date-range summary is calculated from the shared cloud data
- Missing configured employees such as AE01 are restored from the central Employee Master

## Security
The SQL policies allow authenticated users to access the shared tracker data. Do not use the Supabase `service_role` or secret key in the website. For a production organization, separate admin/user roles can be added later.

## Local data migration
When the cloud attendance table is empty, the app detects existing browser-local attendance and asks whether to upload it to the cloud. Export a backup before migration as an additional safeguard.
